...
class MyApp
{
    ...
    function dispatching(array $args = array())
    {
        ...
        else
        {
            ...
            $submodule = $udi[QContext::UDI_SUBMODULE];
            if ($submodule != QContext::UDI_DEFAULT_SUBMODULE && $submodule)
            {
                $class_name .= "{$submodule}_";
                $dir .= "/{$submodule}";
            }
    ...
    function controllerACL($udi)
    {
        ...
        if ($udi[QContext::UDI_SUBMODULE] && $udi[QContext::UDI_SUBMODULE] != QContext::UDI_DEFAULT_SUBMODULE)
        {
            $path .= '/' . $udi[QContext::UDI_SUBMODULE];
        }
        $acl = Q::ini($path);
