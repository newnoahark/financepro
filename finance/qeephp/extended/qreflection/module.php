...
class QReflection_Module
{
    ...
    function controllersName()
    {
        if (is_null($this->_controllers_name))
        {
            ...
            foreach ($files as $file)
            {
                ...
                $submodule = isset($names[1]) ? "{$names[0]}::" : '';
                $controller_name = array_pop($names);
                $this->_controllers_name[] = "{$submodule}{$controller_name}";
            }
        }
     ...
     * @return boolean
     */
    function hasController($controller_name, $submodule = null)
    {
        $udi = strtolower($controller_name);
        $names = explode('::', $controller_name);
        if (!isset($names[1]))
        {
            if ($submodule) $udi = strtolower($submodule) . '::' . $udi;
        }
     ...
     */
    function generateController($controller_name, $submodule = null)
    {
        return $this->controllerGenerator()->generate($controller_name, $submodule);
    }
