...
abstract class QDB_ActiveRecord_Abstract implements QDB_ActiveRecord_Callbacks, QDB_ActiveRecord_Interface, ArrayAccess {
	...
	function reload() {
		...
		if (self::$_meta[$this->_class_name]->idname_count > 1) {
			...
			// 单一主键
			$where = array(
					reset(self::$_meta[$this->_class_name]->idname) => $this->id() 
			);
		}
	 ...
	 * 批量设置对象的属性值
	 *
	 * 如果指定了 $attr_accessible 参数，则会忽略 ActiveRecord 类的 attr_accessible 和
	 * attr_protected 设置。
	 *
	 ...
	 *        	指定哪些属性允许设置
	 * @param boolean $_from_storage
	 *        	内部参数
	 * @param boolean $_ignore_readonly
	 *        	内部参数
	 *        	
	 * @return QDB_ActiveRecord_Abstract 连贯接口
	 */
	...
	function __get($prop_name) {
		...
		if (!isset($this->_props[$prop_name]) && $config['assoc']) {
			// 如果属性是一个关联，并且没有值，则通过 QDB_ActiveRecord_Meta::relatedObjects()
			// 获得关联对象
		...
		foreach ($meta->belongsto_props as $prop_name => $assoc) {
			...
			if (empty($this->_props[$mapping_name])) {
				if (empty($this->_props[$source_key])) {
					if ($this->_props[$source_key] === $meta->props[$source_key]['default_value'] && !is_null($meta->props[$source_key]['default_value'])) {
	...
	protected function _update($recursion = 99) {
		...
		if (!empty($this->_changed_props)) {
			...
			foreach ($this->_props as $prop_name => $value) {
				...
				if (isset($meta->update_reject[$prop_name]) || ($meta->props[$prop_name]['virtual'] && !isset($meta->table_meta[$prop_name]))) {
					continue;
				}
				// 只有指定为脏状态的属性才更新到数据库
				if (isset($this->_changed_props[$prop_name])) {
