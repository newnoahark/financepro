<?php
// $Id: manytomany.php 1937 2009-01-05 19:09:40Z dualface $
 ...
 * @version $Id: manytomany.php 1937 2009-01-05 19:09:40Z dualface $
 * @package orm
 */
...
class QDB_ActiveRecord_Association_ManyToMany extends QDB_ActiveRecord_Association_Abstract {
	...
	function init() {
		...
		if (empty($p['mid_class'])) {
			...
			$this->mid_table = new QDB_Table(array(
					'name' => $mid_table_name
			));
		}
		else {
	...
	protected function _updateRelationByMeta(QDB_ActiveRecord_Abstract $source, $targets, $recursion) {
		...
		// 取出现有的关联信息
		$source_key_value = $source->{$this->source_key};
		$exists = $this->mid_meta->find(array(
				$this->mid_source_key => $source_key_value
		))->all()->asColl()->query();
		...
		foreach ($targets as $target) {
			...
			if (!$exists->search($this->mid_target_key, $v)) {
				$added[] = $v;
			}
		}
		...
		foreach ($exists as $exist) {
			$v = $exist->{$this->mid_target_key};
			if (!in_array($v, $target_key_values)) {
				$removed[] = $v;
			}
		}
		...
		// 删除多余的关联信息
		if (!empty($removed)) {
			$this->mid_meta->destroyWhere(array(
					$this->mid_target_key => $removed
			));
		}
	...
	protected function _updateRelationByTable(QDB_ActiveRecord_Abstract $source, $targets, $recursion) {
		...
		$source_key_value = $source->{$this->source_key};
		$sql = sprintf('SELECT %s FROM %s WHERE %s = %s', $conn->qid($this->mid_target_key), $conn->qid($this->mid_table->getFullTableName()), $conn->qid($this->mid_source_key), $conn->qstr($source_key_value));
		...
		// 删除多余的关联信息
		if (!empty($remove_mid)) {
			$this->mid_table->delete("{$this->mid_source_key} = ? AND {$this->mid_target_key} IN (?)", $source_key_value, $remove_mid);
		}
