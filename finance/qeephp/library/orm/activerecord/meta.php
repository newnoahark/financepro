...
class QDB_ActiveRecord_Meta implements QDB_ActiveRecord_Callbacks {
	...
	function findByArgs(array $args = array()) {
		...
		if ($this->inherit_base_class && $this->inherit_type_field) {
			...
			$select->where(array(
					$this->inherit_type_field => $this->class_name
			));
		}
	...
	function updateDbWhere() {
		...
		return QDB::getConn()->affectedRows();
	}
		...
		foreach ($this->validations as $prop => $policy) {
			...
			if (is_null($data[$prop])) {
				...
				if (!$policy['policy']['allow_null']) {
					...
					$error[$prop]['not_null'] = $prop . ' not null';
				}
	...
	function relatedObjects(QDB_ActiveRecord_Abstract $obj, $prop_name) {
		...
		switch ($assoc->type) {
			...
			case QDB::BELONGS_TO:
				 ...
				 * @var QDB_Select
				 */
				$select = $target_meta->find(array(
						$assoc->target_key => $source_key_value
				));
				break;
			case QDB::MANY_TO_MANY:
				$assoc->mid_table->init();
				/* @var $assoc QDB_ActiveRecord_Association_ManyToMany */
				$select = $target_meta->find("[{$assoc->target_key}] = [m.{$assoc->mid_target_key}]")->joinInner(array(
						'm' => $assoc->mid_table
				), null, "[{$assoc->mid_source_key}] = ?", $source_key_value);
				break;
			default:
				// LC_MSG: 无效的关联类型 "%s".
		...
		if (!empty($assoc->on_find_where)) {
			if (is_array($assoc->on_find_where)) {
				call_user_func_array(array(
				$select,
				'where'
					), $assoc->on_find_where);
	...
	function addDynamicMethod($method_name, $callback, array $custom_parameters = array()) {
		...
		$this->methods[$method_name] = array(
				$callback,
				$custom_parameters
		);
		return $this;
	...
	function addStaticMethod($method_name, $callback, array $custom_parameters = array()) {
		...
		$this->static_methods[$method_name] = array(
				$callback,
				$custom_parameters
		);
		return $this;
	...
	function setPropSetter($prop_name, $callback, array $custom_parameters = array()) {
		...
		if (isset($this->props[$prop_name])) {
			$this->props[$prop_name]['setter'] = array(
					$callback,
					$custom_parameters
			);
		}
		else {
			$this->addProp($prop_name, array(
					'setter' => $callback,
					'setter_params' => $custom_parameters
			));
		}
	...
	function setPropGetter($name, $callback, array $custom_parameters = array()) {
		...
		if (isset($this->props[$name])) {
			...
			$this->addProp($name, array(
					'getter' => $callback,
					'getter_params' => $custom_parameters
			));
		}
	}
	...
	function addEventHandler($event_type, $callback, array $custom_parameters = array()) {
		...
		$this->callbacks[$event_type][] = array(
				$callback,
				$custom_parameters
		);
		return $this;
	...
	function addExceptionTrap(QDB_ActiveRecord_Abstract $obj, $exception_type, $callback, array $custom_parameters = array()) {
		...
		$obj->__exception_trap[$exception_type][] = array(
				$callback,
				$custom_parameters
		);
		return $this;
	 ...
	 *
	 * $assoc_type 指定了关联的类型，可以是 QDB::BELONGS_TO、QDB::HAS_MANY、QDB::HAS_ONE 或
	 * QDB::MANY_TO_MANY。
	 *
	...
	private function _init1($class) {
		...
		$ref = (array)call_user_func(array(
				$class,
				'__define'
		));
		...
		if (!empty($ref['inherit'])) {
			 ...
			 */
			$base_ref = (array)call_user_func(array(
					$this->inherit_base_class,
					'__define'
			));
