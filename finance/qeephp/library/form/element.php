...
/**
 * 增加方法 control() 用于输出一个表单元素
 * 
 * @author sqlhost
 * @version 1.0.0 2012-4-26
 *         
 */
class QForm_Element {
	...
	function import($data) {
		...
		if ($this->_data_binding && isset($data)) {
	...
	function control() {
		...
		$out = '';
		$out .= Q::control($this->_ui, $this->id, $this->attrs());
		if ($this->_req) {
		...
		if (!$this->isValid()) {
			...
			if (is_array($this->errorMsg()) && count($this->errorMsg())) {
				$out .= '<img src="images/check_error.gif" width="13" height="13"> ';
				$out .= nl2br(h(implode("，", $this->errorMsg())));
			}
		}
		else {
