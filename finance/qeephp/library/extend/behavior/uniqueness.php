<?php
// $Id: uniqueness.php 2435 2009-04-23 09:13:24Z jerry $
 ...
 * @version $Id: uniqueness.php 2435 2009-04-23 09:13:24Z jerry $
 * @package behavior
 */
...
class Model_Behavior_Uniqueness extends QDB_ActiveRecord_Behavior_Abstract
{
    ...
    private function _checkUniqueness(QDB_ActiveRecord_Abstract $obj, QDB_Cond $more_cond = null, $ignore_id = false)
    {
        ...
        foreach ($check_props as $check)
        {
            ...
            {
                $failed[$check] = array($check => $this->_settings['error_messages'][$check]);
            }
            else
            {
                $failed[$check] = array($check => "{$check} duplicated");
            }
        }
