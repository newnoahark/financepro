 ...
 *
 * 该选项可以设置为 cleartext, md5、crypt、sha1、sha2，或任何有效的 PHP 全局函数名。其中，cleartext
 * 表示不加密密码。
 *
 ...
 * @author sqlhost
 *         2012-4-2
 *         绑定并增加方法 validateEmail()，用于注册提交前验证邮箱
 *        
 */
class Model_Behavior_AclUser extends QDB_ActiveRecord_Behavior_Abstract {
	 ...
	 */
	function bind() {
		$this->_addStaticMethod ( 'validateLogin', array (
				$this,
				'validateLogin' 
		) );
		$this->_addDynamicMethod ( 'validateEmail', array (
				$this,
				'validateEmailDyn' 
		) );
		$this->_addStaticMethod ( 'validateUsername', array (
				$this,
				'validateUsername' 
		) );
		$this->_addStaticMethod ( 'validatePassword', array (
				$this,
				'validatePassword' 
		) );
		$this->_addStaticMethod ( 'changePassword', array (
				$this,
				'changePassword' 
		) );
		
		$this->_addDynamicMethod ( 'checkPassword', array (
				$this,
				'checkPasswordDyn' 
		) );
		$this->_addDynamicMethod ( 'changePassword', array (
				$this,
				'changePasswordDyn' 
		) );
		$this->_addDynamicMethod ( 'updateLogin', array (
				$this,
				'updateLoginDyn' 
		) );
		
		if ($this->_settings ['roles_enabled']) {
			$this->_addDynamicMethod ( 'aclRoles', array (
					$this,
					'aclRolesDyn' 
			) );
		}
		
		$this->_addDynamicMethod ( 'aclData', array (
				$this,
				'aclDataDyn' 
		) );
		
		$this->_addEventHandler ( self::AFTER_VALIDATE_ON_CREATE, array (
				$this,
				'_after_validate_on_create' 
		) );
		$this->_addEventHandler ( self::AFTER_VALIDATE_ON_UPDATE, array (
				$this,
	 ...
	 * 验证注册邮箱是否被使用
	 */
	function validateEmailDyn($email) {
		// $db = $this->_meta->table->getConn();
		$result = $this->_meta->find ()->where ( array (
				'email' => $email 
		) )->count ();
	}
	 ...
	 *        	旧密码
	 * @param boolean $ignore_old_password
	 *        	是否忽略对旧密码的检查，默认为 false
	 *        	
	 *        	@throw AclUser_UsernameNotFoundException,
	 *        	AclUser_WrongPasswordException
	 */
	 ...
	 * @endcode
	 *
	 * @param QDB_ActiveRecord_Abstract $member
	 *        	要检查的用户对象
	 * @param string $password
	 *        	检查密码
	 *        	
	 ...
	 */
	function changePasswordDyn(QDB_ActiveRecord_Abstract $member, $new_password, $old_password, $ignore_old_password = false) {
	 ...
	 * 更新用户的登录信息
	 *
	 * 要更新的属性由 update_login_count_prop、update_login_at_prop 和
	 * update_login_ip_prop
	 * 设置指定。
	 *
	 * 用法：
	 * @code php
	...
	function _after_validate_on_create(QDB_ActiveRecord_Abstract $member) {
		...
		/* 验证注册邮箱 */
		if (false) { // if用于今后增加了系统参数“是否验证注册邮箱唯一性”
			if ($this->_meta->find ( array (
					'email' => $member [$this->_settings ['email_prop']] 
			) )->getCount () > 0) {
				Q::loadClass('AclUser_EmailExistException', dirname(__FILE__));
				throw new AclUser_EmailExistException ();
			}
		}
		...
		$this->_meta->addExceptionTrap ( $member, self::CREATE_EXCEPTION, array (
				$this,
