...
class QContext implements ArrayAccess
{
    ...
    // 默认的名字空间
    const UDI_DEFAULT_SUBMODULE  = 'default';
    ...
    private static $_udi_defaults = array(
        ...
        self::UDI_SUBMODULE => self::UDI_DEFAULT_SUBMODULE,
    ...
    function reinit($full_init = false)
    {
        ...
        $udi[self::UDI_SUBMODULE]  = (isset($keys[self::UDI_SUBMODULE]))
                                     ? $_GET[$keys[self::UDI_SUBMODULE]] : null;
     ...
     *
     * @code php
     * submodule::controller/action@module
     * @endcode
     *
     * UDI 字符串中，每一个部分都是可选的。
     ...
     * 'submodule::controller/action'
     * 'submodule::controller@module'
     * 'submodule::controller/action@module'
     * '@module'
     * 'submodule::@module'
     * @endcode
    ...
    {
        return "{$udi[self::UDI_SUBMODULE]}::{$udi[self::UDI_CONTROLLER]}/{$udi[self::UDI_ACTION]}@{$udi[self::UDI_MODULE]}";
    }
    ...
    function normalizeUDI($udi, $return_array = true)
    {
        ...
        if (empty($udi[self::UDI_SUBMODULE]))
        {
            $udi[self::UDI_SUBMODULE] = $this->submodule_name;
        }
    ...
    function changeRequestUDI($udi)
    {
        ...
        $this->submodule_name       = $udi[self::UDI_SUBMODULE];

        if ($this->module_name == self::UDI_DEFAULT_MODULE)
        {
            $this->module_name = null;
        }
        if ($this->submodule_name   == self::UDI_DEFAULT_SUBMODULE)
        {
            $this->submodule_name = null;
        }
        return $this;
    }
    ...
    function getInt($arg) {
    	...
    	return intval($this->$arg) ? intval($this->$arg) : 0;
    }
}
