...
// define('Q_DIR', 'ftp://sqlhost:talent@192.168.1.111/qeephp/library');
...
$ip = isset($onlineipmatches[0]) ? $onlineipmatches[0] : 'unknown';
define('IP', $ip);
// ---------------------------------------------------------
...
// 		echo $filename."\n\r";
	 ...
	 * 清除缓存
	 * 
	 * @author sqlhost 2014-11-11
	 * @param unknown_type $policy
	 * @param unknown_type $backend_class
	 */
	static function emptyCache($policy = null, $backend_class = null) {
...
function _h($text) {
	...
	if (is_array($text)) {
		foreach ($text as $key => $val) {
			$text[$key] = htmlspecialchars($val, ENT_QUOTES);
		}
	}
	else {
...
function _d($text) {
	...
	if (is_array($text)) {
		foreach ($text as $key => $val) {
			$text[$key] = htmlspecialchars_decode($val, ENT_QUOTES);
		}
	}
	else {
...
/**
 * QTranslate->get()方法的简写
 */
function _t($key, $language = '') {

	$obj = Q::registry('QTranslate');
	echo $obj->get($key, $language);
}

function _i($key, $language = '') {
 ...
 * 截取字符串
 */
function c($string, $length, $dot = '...', $charset = 'utf-8') {
