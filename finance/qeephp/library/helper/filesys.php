...
abstract class Helper_Filesys {
	...
	protected $_valid_chars_regex = '.A-Z0-9_ !@#$%^&()+={}\[\]\',~`-';
	 ...
	 * @return void +----------------------------------------------------------
	 */
	static function delFiles($directory) {
	...
	private static function _copyDirectoryRecursive($src, $dst, $base, $extnames, $excludes, $level) {
		...
		while (($file = readdir($folder))) {
			...
			if (self::_validatePath($base, $file, $is_file, $extnames, $excludes)) {
				...
				elseif ($level) {
					self::_copyDirectoryRecursive($path, $dst . DIRECTORY_SEPARATOR . $file, $base . '/' . $file, $extnames, $excludes, $level - 1);
				}
			}
		}
		closedir($folder);
	 ...
	 * @return 包含有效文件名的数组
	 */
	private static function _findFilesRecursive($dir, $base, $extnames, $excludes, $level) {
		...
		while (($file = readdir($handle))) {
			...
			if (self::_validatePath($base, $file, $is_file, $extnames, $excludes)) {
				...
				elseif ($level) {
					$list = array_merge($list, self::_findFilesRecursive($path, $base . '/' . $file, $extnames, $excludes, $level - 1));
				}
			}
		}
		closedir($handle);
	 ...
	 */
	private static function _validatePath($base, $file, $is_file, array $extnames, array $excludes) {
	 ...
	 */
	static function verifyFileName($filename) {

		$file_name = preg_replace('/[^' . self::_valid_chars_regex . ']|\.+$/i', "", basename($filename));
		if (strlen($file_name) == 0 || strlen($file_name) > $this->_max_file_name) {
			return false;
		}
		return true;
	}
	 ...
	 */
	static function getSzie($size) {

		// 获得大小上限
		$unit = strtoupper(preg_replace("/^\d+/", '', $size));
		$multiplier = ($unit == 'M' || $unit == 'MB' ? 1048576 : ($unit == 'K' || $unit == 'KB' ? 1024 : ($unit == 'G' || $unit == 'GB' ? 1073741824 : 1)));
		
		// 判断视频大小
		return intval($size) * $multiplier;
	}
	
	/**
	 * 返回文件选择窗口中需要的文件类型的字符串
	 * 即将 doc,txt,xls转换成*.doc,*.txt,*.xls
	 * @param unknown_type $ext
	 */
	static function getFileOpenDialogExt($ext) {
		$ext = array_filter(explode(',', $ext), 'trim');
		Helper_Array::fill($ext, '*.');
		return implode(';', $ext);
	}
	
	static function randName($filename, $ext) {
		return date('ymdHi') . substr(md5($filename . microtime() . Helper_Common::random2(6)), 8, 16) . "." . $ext;
	}
}
