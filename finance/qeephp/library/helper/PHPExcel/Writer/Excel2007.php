...
class PHPExcel_Writer_Excel2007 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter{
	...
	public function save($pFilename = null){
		...
		if($this->_spreadSheet !== NULL){
			...
			// 如果 $pFilename 是 php://output 或 php://stdout，使它的临时文件...
			...
			$saveDebugLog = PHPExcel_Calculation::getInstance($this->_spreadSheet)->getDebugLog()
				->getWriteDebugLog();
			PHPExcel_Calculation::getInstance($this->_spreadSheet)->getDebugLog()
			...
			// 检索覆盖，并从实例化的 zip 类此方法访问常量值从动态类应工作与所有适当版本的 PHP 创建常量
			...
			// 将 [Content_Types].xml 添加到 ZIP 文件
			...
			// 如果 hasMacros，添加的 vbaProject.bin 文件，证书 file(if exists)，
			...
			// 此工作簿中的自定义 UI 吗？添加它 （"基地"xml 和其他对象 （图片） 和 rels）
			if($this->_spreadSheet->hasRibbon()){
				...
				if($this->_spreadSheet->hasRibbonBinObjects()){
					...
					// the rels for files
					// 文件关联
					$objZip->addFromString($tmpRootPath . '_rels/' . basename($tmpRibbonTarget) . '.rels', $this->getWriterPart('RelsRibbonObjects')
			...
			for($i = 0; $i < $this->_spreadSheet->getSheetCount(); ++$i){
				$objZip->addFromString('xl/worksheets/sheet' . ($i + 1) . '.xml', $this->getWriterPart('Worksheet')
					->writeWorksheet($this->_spreadSheet->getSheet($i), $this->_stringTable, $this->_includeCharts));
				if($this->_includeCharts){
					$charts = $this->_spreadSheet->getSheet($i)
						->getChartCollection();
					if(count($charts) > 0){
						foreach($charts as $chart){
							$objZip->addFromString('xl/charts/chart' . ($chartCount + 1) . '.xml', $this->getWriterPart('Chart')
								->writeChart($chart));
							$chartCount++;
						}
					}
			...
			for($i = 0; $i < $this->_spreadSheet->getSheetCount(); ++$i){
				...
				$objZip->addFromString('xl/worksheets/_rels/sheet' . ($i + 1) . '.xml.rels', $this->getWriterPart('Rels')
					->writeWorksheetRelationships($this->_spreadSheet->getSheet($i), ($i + 1), $this->_includeCharts));
				
				$drawings = $this->_spreadSheet->getSheet($i)
					->getDrawingCollection();
				$drawingCount = count($drawings);
				if($this->_includeCharts){
					$chartCount = $this->_spreadSheet->getSheet($i)
						->getChartCount();
				}
				...
				if(($drawingCount > 0) || ($chartCount > 0)){
					...
					$objZip->addFromString('xl/drawings/_rels/drawing' . ($i + 1) . '.xml.rels', $this->getWriterPart('Rels')
						->writeDrawingRelationships($this->_spreadSheet->getSheet($i), $chartRef1, $this->_includeCharts));
					
					// Drawings
					$objZip->addFromString('xl/drawings/drawing' . ($i + 1) . '.xml', $this->getWriterPart('Drawing')
						->writeDrawings($this->_spreadSheet->getSheet($i), $chartRef2, $this->_includeCharts));
				}
			...
			if($objZip->close() === false){
				throw new PHPExcel_Writer_Exception("无法关闭 zip 文件 ：$pFilename.");
			}
			
			// If a temporary file was used, copy it to the correct file stream
			// 如果使用了一个临时文件，将它复制到正确的文件流
			if($originalFilename != $pFilename){
				if(copy($pFilename, $originalFilename) === false){
					throw new PHPExcel_Writer_Exception("无法复制临时 zip 文件： $pFilename to $originalFilename.");
				}
				@unlink($pFilename);
			}
		}else{
			throw new PHPExcel_Writer_Exception("PHPExcel 未分配的对象.");
		}
	}
