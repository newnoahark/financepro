 ...
 * Additional projects that may be used:
 * http://sourceforge.net/projects/debugobject/
 ...
 * but not accurately.
 * This tag_start gets counted AFTER \r\n have been crushed out, and after the
 * remove_noice calls so it will not reflect the REAL position of the tag in the
 * source,
 * it will almost always be smaller by some amount.
 * We use this to determine how far into the file the tag in question is. This
 * "percentage will never be accurate as the $dom->size is the "real" number of
 * bytes the dom was created from.
 ...
 * plaintext implies text in the innertext of a node. text implies that the tag
 * is a text node.
...
abstract class simpleHtmlDomDom {
...
class simple_html_dom_node {
	...
	/*
	 * function is_utf8($string) { //this is buggy return
...
// use cUrl to replace function curl_get_contents
function curl_get_contents($url)
{
	...
	//	curl_setopt($curl, CURLOPT_HTTP_VERSION, $useragent);
	//	curl_setopt($curl, CURLOPT_USERAGENT, _USERAGENT_);
	//	curl_setopt($curl, CURLOPT_REFERER,_REFERER_);
	//	curl_setopt($curl, CURLOPT_HEADER, 0);
	curl_setopt ($curl, CURLOPT_REFERER, 'http://www.baidu.com/baidu?word=s');
