...
abstract class QValidator{
	 ...
	 * 长度区间
	 */
	static function validate_between_length($value, $min, $max){
	 ...
	 * @return boolean
	 */
	static function validate_is_mobile($mobile){
	...
	static function validate_is_time($value){
		...
		if($test === -1 || $test === false || date('H:i:s', $test) != ($count == 2 ? $value . ":00" : $value)){
			return false;
		}
	 ...
	 * @return boolean
	 */
	static function validate_is_imgcode($imgcode){

		return Helper_ImgCode::isValid($imgcode);
	}
}
