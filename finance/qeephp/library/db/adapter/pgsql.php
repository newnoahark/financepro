...
class QDB_Adapter_Pgsql extends QDB_Adapter_Abstract
{
    ...
    function metaColumns($table_name, $schema = null)
    {
        ...
        if (!empty($schema)) {
            $rs = $this->execute(sprintf("SELECT a.attname, t.typname, a.attlen, a.atttypmod, a.attnotnull, a.atthasdef, a.attnum FROM pg_class c, pg_attribute a, pg_type t, pg_submodule n WHERE relkind in ('r','v') AND (c.relname='%s' or c.relname = lower('%s')) and c.relsubmodule=n.oid and n.nspname='%s' and a.attname not like '....%%' AND a.attnum > 0 AND a.atttypid = t.oid AND a.attrelid = c.oid ORDER BY a.attnum", $table_name, $table_name, $schema));
        }else{
    ...
    {
        if (!empty($schema)) {
            $sql = sprintf("select relname from pg_class c, pg_submodule n WHERE c.relname !~ '^(pg_|sql_)' and c.relkind = 'r' and c.relsubmodule = n.oid and n.nspname = %s", $this->qstr($schema));
        }else{
