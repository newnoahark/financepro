...
abstract class QDB_Adapter_Abstract {
	 ...
	 *        占位符样式
	 * @param boolean $return_parameters_count
	 *        是否返回占位符个数
	 *        
	 * @return string array
	 */
	 ...
	 * @param array $row
	 *        要插入的记录数据
	 * @param string|array $restricted_fields
	 *        限定只使用哪些字段
	 */
	...
	/**
	 * 进行限定范围的查询，并且返回 QDB_Result_Abstract 对象，出错时抛出异常 使用 selectLimit()，可以限定
	 * SELECT 查询返回的结果集的大小。
	 * $length 参数指定结果集最多包含多少条记录。
	 * 而 $offset参数则指定在查询结果中，从什么位置开始提取记录。
	 ...
	 * 注意：$offset 参数是从 0 开始计算的。
	 * 因此 $offset 为 59 时，实际上是从第 60 条记录开始提取。
	 * selectLimit() 并不直接返回结果集，而是返回
	 * QDB_Result_Abstract 对象。 因此需要调用 QDB_Result_Abstract 对象的 fetchAll()
	 * 等方法来获得数据。
	 * example: 
	 * @code $sql = "SELECT * FROM posts WHERE created > ?
	 * ORDER BY post_id DESC"; $length = 20; $offset = 0; $current = time() - 60
	 * * 60 * 24 * 15; // 查询创建时间在 15 天内的记录 $handle = $dbo->selectLimit($sql,
	 * $offset, $length, array($current)); $rowset = $handle->fetchAll();
	 * $handle->free(); @endcode @param string $sql @param int $offset @param
	 * int $length @param array $inputarr @return QDB_Result_Abstract
	 */
