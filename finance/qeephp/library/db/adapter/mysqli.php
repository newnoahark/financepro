 ...
 * @package database
 */
class QDB_Adapter_Mysqli extends QDB_Adapter_Abstract {
	...
	function connect($pconnect = false, $force_new = false) {
		...
		if ($pconnect) {
			$this->_conn = mysqli_connect($host, $this->_dsn['login'], $this->_dsn['password'], $this->_dsn['database']);
		}
		else {
			$this->_conn = mysqli_connect($host, $this->_dsn['login'], $this->_dsn['password'], $this->_dsn['database']);
		}
		...
		if (! is_object($this->_conn)) {
			throw new QDB_Exception('CONNECT DATABASE', mysqli_error($this->_conn), mysqli_errno($this->_conn));
		}
...
// 		}
	...
	function qstr($value) {
		if (is_array($value)) {
			/**
			 * 修改此处对数组的处理方式，因为如果是数组的话，一般是enum属于，需要将数组转换成mysql的enum格式
			 * 
			 * @author sqlhost
			 * @version 1.0.0
			 * 2012-4-13
			...
			return $this->qstr($temp);
			/*
			 * foreach ($value as $offset => $v) { $value[$offset] =
			 * $this->qstr($v); } return $value;
			 */
		}
		if (is_int($value)) {
			return $value;
...
// 		dump($sql);
		...
		if ($result) {
			return new QDB_Result_Mysqli($result, $this->_fetch_mode);
		}
		elseif ($result) {
			$this->_last_err = null;
			$this->_last_err_code = null;
			...
			$this->_last_err = mysqli_error($this->_conn);
			$this->_last_err_code = mysqli_errno($this->_conn);
	...
	function metaColumns($table_name) {
		...
		while (($row = $rs->fetchRow())) {
			 ...
			 * 增加set类型的支持
			 */
			elseif (preg_match('/^(set)\((.*)\)$/i', $type, $query_arr)) {
				$field['type'] = $query_arr[1];
				$arr = explode(",", $query_arr[2]);
				$field['set'] = $arr;
				$zlen = max(array_map("strlen", $arr)) - 2; // PHP >= 4.0.6
				$field['length'] = - 1; // ($zlen > 0) ? $zlen : 1;
			}
			// -
			else {
				$field['type'] = $type;
